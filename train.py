...
def preprocess_raw_image(x, enc_type):
    ...
    if 'clip' in enc_type:
        ...
        x = x / 255.
        x = Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)(x)
        x = torch.nn.functional.interpolate(x, 224 * (resolution // 256), mode='bicubic')  #torch.Size([32, 3, 224, 224])
    ...
    return x  #torch.Size([32, 3, 224, 224])
...
def main(args):    
    ...
    #             # 从 patch_embed 推导嵌入维度，适配 DINOv3
    #             embed_dim = encoder.patch_embed.proj.weight.shape[-1]
    #             z_dims.append(embed_dim)
    #     else:
    #         z_dims = [encoder.embed_dim for encoder in encoders] if args.enc_type != 'None' else [0]
        ...
        for raw_image, x, y in train_dataloader:
            ...
            with torch.no_grad():
                ...
                with accelerator.autocast():
                    for encoder, encoder_type, arch in zip(encoders, encoder_types, architectures):
                        raw_image_ = preprocess_raw_image(raw_image, encoder_type)   #torch.Size([32, 4, 32, 32])
